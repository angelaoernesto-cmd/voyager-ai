...
// ─────────────────────────────────────────────────────────────────────────────
// THEMES  –  Light mode is genuinely light. Dark mode is dark.
// The calendar background follows the theme too.
// ─────────────────────────────────────────────────────────────────────────────
const LIGHT = {
  bg:        "#F5F0E8",
  bgCard:    "#FFFFFF",
...
async function callAI(msgs) {
  const r = await fetch("https://api.anthropic.com/v1/messages",{
    ...
    body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1400,
      system:"Eres experto planificador de viajes. Responde en español. SOLO JSON válido sin markdown.",
      messages: msgs })
  });
...
const CSS = `@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,600;9..40,700&family=Playfair+Display:wght@700;900&display=swap');
...
function CitySheet({city,onClose,onBack,onUpdate,T}){
  ...
  useEffect(()=>{
    if(!city.desc&&!city.attractions?.length){
      sL(true);
      callAI([{role:"user",content:`Información turística de ${city.name}${city.country?", "+city.country:""}.
...
function SetupWizard({T,onCancel,onDone}){
  ...
  const getSugg=useCallback(async()=>{
    ...
    try{
      const t=await callAI([{role:"user",content:`Destino: ${dest}. Mes: ${MONTHS[month]} ${year}.
